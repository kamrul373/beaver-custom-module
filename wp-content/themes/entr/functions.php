...
function entr_theme_stylesheets() {
    ...
    wp_enqueue_style('mmenu-light', get_template_directory_uri() . '/assets/css/mmenu-light.min.css', array(), ENTR_VERSION);
    // Theme stylesheet.
...
function entr_theme_js() {
    ...
    wp_enqueue_script('mmenu', get_template_directory_uri() . '/assets/js/mmenu-light.min.js', array('jquery'), ENTR_VERSION, true);
}
...
function entr_word_counter_label($label) {
	$label = "Total Words of this post :";
	return $label;
}
add_filter("wordcounter_label", 'entr_word_counter_label');

function entr_wordcounter_tag($tag) {
	return 'strong';
}

add_filter("wordcounter_tag", 'entr_wordcounter_tag');
